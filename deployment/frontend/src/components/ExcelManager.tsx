...
const ExcelManager: React.FC = () => {
  ...
  const downloadScope = user?.role === 'admin' ? t('scope.allStudents') : t('scope.agencyStudents');
  ...
  const handleTemplateDownload = async () => {
    if (user?.role !== 'admin' && user?.role !== 'teacher') {
      setError(t('error.noTemplatePermission'));
      return;
    }
    ...
    try {
      ...
      setError(extractErrorMessage(error, t('error.templateDownloadError')));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Paper elevation={2} sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 2 }}>
        ...
        />
        <Chip 
          label={`${t('label.downloadScope')}: ${downloadScope}`}
          color="info"
          size="small"
          sx={{ ml: 1 }}
        />
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          ...
          <Grid item xs={12} md={6}>
            <Button
              ...
              fullWidth
            >
              {t('button.downloadConsultationRecords')}
            </Button>
          </Grid>
        </Grid>
      ) : (
        ...
        <Box>
          ...
          <Alert severity="info" icon={false}>
            ...
            <ol style={{ margin: '8px 0', paddingLeft: '20px' }}>
              <li>{t('button.downloadTemplate')}</li>
