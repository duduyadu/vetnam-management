...
const Consultations: React.FC = () => {
  ...
  useEffect(() => {
    loadConsultations();
  }, [page, rowsPerPage, searchTerm, smartFilter]);
  ...
  const applySmartFilter = (filterType: string | null) => {
      ...
      switch (filterType) {
        case 'today':
          if (consultDate.toDateString() === today.toDateString()) {
            filtered.push(consultation.consultation_id);
          }
          break;
        case 'this_week':
          ...
          if (consultDate >= weekStart && consultDate <= weekEnd) {
            filtered.push(consultation.consultation_id);
          }
          break;
        case 'this_month':
          if (consultDate.getMonth() === today.getMonth() && 
              consultDate.getFullYear() === today.getFullYear()) {
            filtered.push(consultation.consultation_id);
          }
          break;
        case 'academic':
          if (consultation.consultation_type === 'academic') {
            filtered.push(consultation.consultation_id);
          }
          break;
      }
    ...
    setSelectedConsultations(filtered);
  };
        ...
        <Paper sx={{ p: 2, mb: 2 }}>
          <Box sx={{ display: 'flex', gap: 2, flexDirection: 'column' }}>
            <TextField
              fullWidth
              variant="outlined"
              placeholder={t('placeholder.searchConsultation')}
              value={searchTerm}
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              <Typography variant="body2" sx={{ display: 'flex', alignItems: 'center', mr: 1 }}>
                <FilterList sx={{ mr: 0.5 }} /> {t('consultation.quickSelection')}:
              </Typography>
              <ToggleButtonGroup
                value={smartFilter}
                exclusive
              ...
              >
                <ToggleButton value="today">
                  <DateRange sx={{ mr: 0.5 }} /> {t('consultation.todayConsultations')}
                </ToggleButton>
                <ToggleButton value="this_week">
                  <DateRange sx={{ mr: 0.5 }} /> {t('consultation.thisWeekConsultations')}
                </ToggleButton>
                <ToggleButton value="this_month">
                  <DateRange sx={{ mr: 0.5 }} /> {t('consultation.thisMonthConsultations')}
                </ToggleButton>
                <ToggleButton value="academic">
                  <School sx={{ mr: 0.5 }} /> {t('consultation.academicConsultations')}
                </ToggleButton>
              </ToggleButtonGroup>
        ...
        {batchDownloading && (
          <Box sx={{ width: '100%', mb: 2 }}>
            ...
            <Typography variant="caption" sx={{ mt: 1 }}>
              {t('text.downloadProgress')} {Math.round(downloadProgress)}%
            </Typography>
          </Box>
        )}
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>{t('table.consultationDate')}</TableCell>
                <TableCell>{t('table.studentName')}</TableCell>
                <TableCell>{t('table.studentCode')}</TableCell>
                <TableCell>{t('table.consultationType')}</TableCell>
                <TableCell>{t('table.counselor')}</TableCell>
                <TableCell>{t('table.nextConsultation')}</TableCell>
